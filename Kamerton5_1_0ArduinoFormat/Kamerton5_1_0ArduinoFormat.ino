/*
 ...
 Автор:       - Мосейчук А.В.
 Версия: Измененная версия от 12.08.2016г.
 ...
 - подсчет контролных сумм, связь с "Аудио1" и "Аудио2",
 - Расширение MCP23017
 - модуль реле,
 - чтение всех портов,
...
const unsigned int adr_reg40014      PROGMEM       =    1052;  // Модель платы Аудио1 или Аудио2
...
const  int adr_vesion_Audio1_2                 = 750;          // 750 Адрес хранения версии настроек порога по умолчанию.

int por_int_buffer[40] ;                                       // Буфер хранения временной информации уровней порогов
const unsigned int porog_default1[]    PROGMEM  = {             // 270 ячеек
  //++++++++++++++++  Test headset instructor ++++++++++++++++++++++++++++
  31,  // [1] resistor(1, 25);     Установить уровень сигнала 30 мв
  31,  // [2] resistor(2, 25);     Установить уровень сигнала 30 мв
  ...
  0,   //[23]
  0,   //[24]
  80,  //[25] min measure_vol_max(analog_LineL,    40224,224,100); уровень сигнала на выходе LineL
  90,  //[26] max measure_vol_max(analog_LineL,    40224,224,150); уровень сигнала на выходе LineL
  0,   //[27]
  0,   //[28]
  50,  //[29] min measure_vol_max(analog_mag_radio,40333,333,40) ; уровень сигнала на выходе mag phone
  60,  //[30] max measure_vol_max(analog_mag_radio,40333,333,100); уровень сигнала на выходе mag phone
  50,  //[31] min measure_vol_max(analog_mag_phone,40226,226,80) ; уровень сигнала на выходе mag phone
  60,  //[32] max measure_vol_max(analog_mag_phone,40226,226,120); уровень сигнала на выходе mag phone
  0,   //[33]
  0,   //[34]
  0,   //[35]
  0,   //[36]
  ...
  31,  //[39] resistor(1, 30);   Установить уровень сигнала 30 мв
  31,  //[40] resistor(2, 30);   Установить уровень сигнала 30 мв
  ...
  0,   //[61]
  0,   //[62]
  80,  //[63] min measure_vol_max(analog_LineL,    40227,227,200); уровень сигнала на выходе LineL
  90,  //[64] max measure_vol_max(analog_LineL,    40227,227,200); уровень сигнала на выходе LineL
  0,   //[65]
  0,   //[66]
  50,  //[67] min  measure_vol_max(analog_mag_radio
  60,  //[68] max  measure_vol_max(analog_mag_radio
  50,  //[69] min  measure_vol_max(analog_mag_phone,40229,229,200); уровень сигнала на выходе mag phone
  60,  //[70] max  measure_vol_max(analog_mag_phone,40229,229,200); уровень сигнала на выходе mag phone
  0,   //[71]
  0,   //[72]
  0,   //[73]
  0,   //[74]
  ...
  //++++++++++++++++  Test MTT ++++++++++++++++++++++++++++
  226, //[77] resistor(1, 200);  Установить уровень сигнала 60 мв
  226, //[78] resistor(2, 200);  Установить уровень сигнала 60 мв
  ...
  95,  //[103] min measure_vol_max(analog_LineR,     40261,261,70);  "Test MTT ** Signal LineR
  110,  //[104] max measure_vol_max(analog_LineR,     40261,261,70);  "Test MTT ** Signal LineR
  0,   //[105]
  0,   //[106]
  60,  //[107]
  75,  //[108]
  ...
  0,   //[141]
  0,   //[142]
  50,  //[143] min  measure_vol_max(analog_mag_radio
  60,  //[144] max  measure_vol_max(analog_mag_radio
  50,  //[145]
  60,  //[146]
  0,   //[147]
  0,   //[148]
...
const unsigned int porog_default2[]    PROGMEM  = {             // 270 ячеек
  //++++++++++++++++  Test headset instructor ++++++++++++++++++++++++++++
  13,  // [1] resistor(1, 25);     Установить уровень сигнала 85 мв
  13,  // [2] resistor(2, 25);     Установить уровень сигнала 85 мв
  8,   // [3] measure_vol_min(analog_FrontL,   40230,230,18);  уровень сигнала на выходе FrontL
  8,   // [4] measure_vol_min(analog_FrontL,   40230,230,18);  уровень сигнала на выходе FrontL
  8,   // [5] measure_vol_min(analog_FrontR,   40231,231,18);  уровень сигнала на выходе FrontR
  8,   // [6] measure_vol_min(analog_FrontR,   40231,231,18);  уровень сигнала на выходе FrontR
  ...
  0,   //[27]
  0,   //[28]
  90,  //[29] min measure_vol_max(analog_mag_radio,40333,333,40) ; уровень сигнала на выходе mag phone
  110,  //[30] max measure_vol_max(analog_mag_radio,40333,333,100); уровень сигнала на выходе mag phone
  90,  //[31] min measure_vol_max(analog_mag_phone,40226,226,80) ; уровень сигнала на выходе mag phone
  110,  //[32] max measure_vol_max(analog_mag_phone,40226,226,120); уровень сигнала на выходе mag phone
  0,   //[33]
  0,   //[34]
  0,   //[35]
  0,   //[36]
  ...
  0,   //[38]
  //++++++++++++++++  Test headset dispatcher ++++++++++++++++++++++++++++
  13,  //[39] resistor(1, 30);   Установить уровень сигнала 85 мв
  13,  //[40] resistor(2, 30);   Установить уровень сигнала 85 мв
  ...
  0,   //[65]
  0,   //[66]
  90,  //[67] min  measure_vol_max(analog_mag_radio
  110,  //[68] max  measure_vol_max(analog_mag_radio
  90,  //[69] min  measure_vol_max(analog_mag_phone,40229,229,200); уровень сигнала на выходе mag phone
  110,  //[70] max  measure_vol_max(analog_mag_phone,40229,229,200); уровень сигнала на выходе mag phone
  0,   //[71]
  0,   //[72]
  0,   //[73]
  0,   //[74]
  ...
  //++++++++++++++++  Test MTT ++++++++++++++++++++++++++++
  170, //[77] resistor(1, 200);  Установить уровень сигнала 60 мв
  170, //[78] resistor(2, 200);  Установить уровень сигнала 60 мв
  ...
  90,  //[104] max measure_vol_max(analog_LineR,     40261,261,70);  "Test MTT ** Signal LineR
  0,   //[105]
  0,   //[106]
  95,  //[107]
  110, //[108]
  ...
  0,   //[141]
  0,   //[142]
  95,  //[143] min  measure_vol_max(analog_mag_radio
  110,  //[144] max  measure_vol_max(analog_mag_radio
  95 ,  //[145]
  110,  //[146]
  0,   //[147]
  0,   //[148]
...
const char  txt_message4[]    PROGMEM            = "Signal headset instructor microphone 0,5V   ON"      ;
...
const char  txt_message13[]   PROGMEM            = "Signal headset dispatcher microphone  0,5V  ON"      ;
...
const char  txt_message49[]   PROGMEM            = "Signal GGS  FrontL, FrontR   1.0V           ON"      ;
const char  txt_message50[]   PROGMEM            = " ****** Test Radio1 start! ******"                   ;
const char  txt_message51[]   PROGMEM            = "Signal Radio1 0.5V    LFE                   ON"      ;
const char  txt_message52[]   PROGMEM            = " ****** Test Radio2 start! ******"                   ;
const char  txt_message53[]   PROGMEM            = "Signal Radio1 0.5V    Center                ON"      ;
const char  txt_message54[]   PROGMEM            = " ****** Test microphone start! ******"               ;
const char  txt_message55[]   PROGMEM            = "Signal microphone 3  mV                     ON"      ;
...
const char  txt_message61[]   PROGMEM            = "Power module Audio                              - "  ;
...
const char  txt_message71[]   PROGMEM            = "Signal GGS  FrontL, FrontR                  OFF"     ;
const char  txt_message72[]   PROGMEM            = "Command radioperedacha  ON "                         ;
const char  txt_message73[]   PROGMEM            = "Command GGS mute  ON "                               ;
const char  txt_message74[]   PROGMEM            = "Signal headset instructor microphone 85mV   ON"      ;
const char  txt_message75[]   PROGMEM            = "Signal headset dispatcher microphone 85mV   ON"      ;
const char  txt_message76[]   PROGMEM            = "Signal MTT microphone 0.85V                 ON"      ;
const char  txt_message77[]   PROGMEM            = "Signal GGS  FrontL, FrontR   1.0V           ON"      ;
const char  txt_message78[]   PROGMEM            = "Signal Radio1 1.0V    LFE                   ON"      ;
const char  txt_message79[]   PROGMEM            = "Signal Radio1 1.0V    Center                ON"      ;
...
const char  txt_error39[]  PROGMEM               = "Sensor Mag                  XP2 - 2         ON~ - ";
...
const char  txt_error49[]  PROGMEM               = "Sensor Mag                  XP2 - 2         OFF - ";
...
const char  txt_error129[]  PROGMEM              = "Test Microphone ** Signal mag radio         ON~ - ";
...
const char* const table_message[] PROGMEM =
{
  ...
  txt_message74,                                // "Signal headset instructor microphone 85mV   ON"      ;
  txt_message75,                                // "Signal headset dispatcher microphone 85mV   ON"      ;
  txt_message76,                                // "Signal MTT microphone 0.85V                 ON"      ;
  txt_message77,                                // "Signal GGS  FrontL, FrontR   1.0V           ON"      ;
  txt_message78,                                // "Signal Radio1 1.0V    LFE                   ON"      ;
  txt_message79                                 // "Signal Radio1 1.0V    Center                ON"      ;
};
...
const char* const string_table_err[] PROGMEM =
{
  ...
  txt_error39,                                  // "Sensor Mag                  XP2 - 2                         ON~ - "
  ...
  txt_error49,                                  // "Sensor Mag                  XP2 - 2                         OFF - ";
...
void FileOpen()
{
  ...
  else
  {
	...
	else if(regBank.get(40014)== 2)
	{
		myFile.print ("Report of test module Audio-2 N ");
    }
	else
	{
		myFile.print ("Audio module type is not defined N ");
	}
  ...
  if (test_n != 0)
  {
    ...
    switch (test_n)
    {
      ...
      case 53:
        ...
        break;
      case 54:
        model_Audio1_2();                         // Запрос версии подключенного модуля Аудио1/2
        break;
      case 55:
		read_version_default();                   // Запрос версии порогов по умолчанию согласно версии модуля Аудио1/2
		break;
      case 56:
		i2c_eeprom_write_byte(deviceaddress, adr_vesion_Audio1_2, 1);  // Записать признак версии порогов  модуля Аудио1
		regBank.set(adr_control_command, 0); 
		break;
      case 57:
...
void sensor_all_off()
{
  ...
  else
  {
	...
	if(regBank.get(40014)==1)
	{
	  if (bitRead(i52, 0) != 0)                                                 // sensor подключения Mag
	  {
		...
		regcount_err++;                                                         // увеличить счетчик всех ошибок
		regBank.set(adr_reg_count_err, regcount_err);                           // Сохранить данные счетчика всех ошибок
		strcpy_P(buffer, (char*)pgm_read_word(&(string_table_err[49])));       //
		myFile.print(buffer);                                                   //
		strcpy_P(buffer, (char*)pgm_read_word(&(table_message[0])));            // "    Error! - ";
		myFile.print(buffer);                                                   // "    Error! - ";
		myFile.println(regcount);                                               // Показания счетчика ошибок
	  ...
	  else
	  {
		if (test_repeat == false)
		{
		  strcpy_P(buffer, (char*)pgm_read_word(&(string_table_err[49])));   // "Sensor headset instructor 2         XP1- 16 HeS2Rs          OFF - ";
		  myFile.print(buffer);                                               //
		  strcpy_P(buffer, (char*)pgm_read_word(&(table_message[1])));        // "Pass";
  ...
  if(regBank.get(40014)== 1)
	{
	  ...
	  if (bitRead(i52, 0) == 0)                                                    //  sensor подключения Mag
	  {
		regcount = regBank.get(40340);                                             // адрес счетчика ошибки sensor подключения Mag
		regcount++;                                                                // увеличить счетчик ошибок sensor подключения Mag
		regBank.set(40340, regcount);                                              // адрес счетчика ошибки sensor подключения Mag
		regBank.set(340, 1);                                                       // установить флаг ошибки sensor подключения Mag
		regBank.set(120, 1);                                                       // установить общий флаг ошибки
		strcpy_P(buffer, (char*)pgm_read_word(&(string_table_err[39])));          //
		myFile.print(buffer);                                                      //
		strcpy_P(buffer, (char*)pgm_read_word(&(table_message[0])));               // "    Error! - ";
		myFile.print(buffer);                                                      // "    Error! - ";
		myFile.println(regcount);                                                  // Показания счетчика ошибок
	  ...
	  else
	  {
		if (test_repeat == false)
		{
		  ...
		  myFile.print(buffer);                                                    //
		  strcpy_P(buffer, (char*)pgm_read_word(&(table_message[1])));             // "Pass";
		  if (test_repeat == false) myFile.println(buffer);                        // "Sensor Mag включен  - Pass
		}
	  }
    }
  /*
...
void test_headset_instructor()
{
  ...
  resistor(1, por_int_buffer[1]);                                                 // Установить уровень сигнала на вход
  resistor(2, por_int_buffer[2]);                                                 // Установить уровень сигнала на вход
  regBank.set(2, 1);                                                              // Подать сигнал на вход микрофона инструктора  Mic2p
  UpdateRegs();                                                                   // Выполнить команду
  delay(300);
  if(vers_ini == 2)
  {
	 strcpy_P(buffer, (char*)pgm_read_word(&(table_message[74])));                // "Signal headset instructor microphone 85mV   ON"      ;
  }
  else
  {
     strcpy_P(buffer, (char*)pgm_read_word(&(table_message[4])));                  // "Signal headset instructor microphone 3mv     ON"            ;
  }
  if (test_repeat == false)  myFile.println(buffer);                               // "Signal headset instructor microphone 3mv     ON"            ;
  ...
  regBank.set(27, 1);                                                       // XP1- 16 HeS2Rs  Отключить сенсор инструктора c 2  наушниками
  UpdateRegs();                                                             //
  delay(300);
...
void test_headset_dispatcher()
{
  ...
  if(vers_ini == 2)
  {
	 strcpy_P(buffer, (char*)pgm_read_word(&(table_message[75])));                // "Signal headset dispatcher microphone 85mV   ON"      ;
  }
     else
  {
     strcpy_P(buffer, (char*)pgm_read_word(&(table_message[13])));                // "Signal headset dispatcher microphone  0.5v  ON"            ;
  }
...
void test_MTT()
{
  ...
  resistor(1, por_int_buffer[1]);                                                                      // Установить уровень сигнала 0.85в
  resistor(2, por_int_buffer[2]);                                                                      // Установить уровень сигнала 0.85в
  regBank.set(3, 1);                                                                                   // Включить сигнал на вход микрофона трубки Mic3p
  UpdateRegs();                                                                                        // Выполнить команду
  delay(100);
  if(vers_ini == 2)
  {
	 strcpy_P(buffer, (char*)pgm_read_word(&(table_message[76])));                                     // "Signal MTT microphone 0.85v                    ON"      ;
  }
  else
  {
     strcpy_P(buffer, (char*)pgm_read_word(&(table_message[33])));                                     // "Signal MTT microphone 1.0 v                    ON"            ;
  }
  ...
  if(vers_ini == 2)
  {
  resistor(1, 180);                                                                                      // Установить уровень сигнала 1.0 в
  resistor(2, 180); 
  }
  else
  {
  resistor(1, 185);                                                                                      // Установить уровень сигнала 1.0 в
  resistor(2, 185); 
  }
...
void test_GG_Radio1()
{
  ...
  if(vers_ini == 2)
  {
	 strcpy_P(buffer, (char*)pgm_read_word(&(table_message[78])));                                      // "Signal Radio1 1.0V    LFE                    ON"      ;
  }
  else
  {
  strcpy_P(buffer, (char*)pgm_read_word(&(table_message[51])));                                         // "Signal Radio1 0.5V    LFE                   ON"            ;
  }
  if (test_repeat == false) myFile.println(buffer);                               // "Signal Radio1      LFE                   ON"            ;
...
void test_GG_Radio2()
{
  ...
  if(vers_ini == 2)
  {
	 strcpy_P(buffer, (char*)pgm_read_word(&(table_message[79])));                                      // "Signal Radio2 1.0V    LFE                    ON"      ;
  }
  else
  {
     strcpy_P(buffer, (char*)pgm_read_word(&(table_message[53])));                                      // "Signal Radio2 0.5V    LFE                   ON"            ;
  }
  if (test_repeat == false) myFile.println(buffer);                                                     // "Signal Radio2     LFE                       ON"            ;
...
void test_power()
{
  ...
  if (read_reg_eeprom(adr_reg40493) * 2.963 / 100 < 11 || read_reg_eeprom(adr_reg40493) * 2.963 / 100 > 13)
  {
    ...
    myFile.print(" | power ");
    myFile.print(read_reg_eeprom(adr_reg40493) * 2.963 / 100);
    myFile.print(regcount);
    regBank.set(120, 1);
  ...
  if (read_reg_eeprom(adr_reg40494) * 2.963 / 100 < 11 || read_reg_eeprom(adr_reg40494) * 2.963 / 100 > 13)
  {
    if (test_repeat)                                          // Параметры в норме. Тест одиночный
    {
      file_print_date();
    ...
    myFile.print(regcount);
    myFile.print(" | power ");
    myFile.print(read_reg_eeprom(adr_reg40494) * 2.963 / 100);
    myFile.print(regcount);
    regBank.set(120, 1);
    myFile.println("v / porog min 11v - max 13v");
  }
  ...
  if (read_reg_eeprom(adr_reg40495) * 2.963 / 100 < 11 || read_reg_eeprom(adr_reg40495) * 2.963 / 100 > 13)
  {
    if (test_repeat)                                          // Параметры в норме. Тест одиночный
    {
      file_print_date();
    ...
    myFile.print(regcount);
    myFile.print(" | power ");
    myFile.print(read_reg_eeprom(adr_reg40495) * 2.963 / 100);
    myFile.print(regcount);
    regBank.set(120, 1);
    myFile.println("v / porog min 11v - max 13v");
  }
  ...
  if (read_reg_eeprom(adr_reg40496) * 2.963 / 100 < 11 || read_reg_eeprom(adr_reg40496) * 2.963 / 100 > 13)
  {
    if (test_repeat)                                          // Параметры в норме. Тест одиночный
    {
      file_print_date();
    ...
    myFile.print(regcount);
    myFile.print(" | power ");
    myFile.print(read_reg_eeprom(adr_reg40496) * 2.963 / 100);
    myFile.print(regcount);
    regBank.set(120, 1);
    myFile.println("v / porog min 11v - max 13v");
  }
...
void test_video()
{
  ...
  regs_out[0] = 0x2B;                                                  // Код первого байта подключения к Камертону 43
  regs_out[1] = 0x84;                                                  //
  regs_out[2] = 0x7F;                                                  // Уровень яркости максимальный
  delay(200);
  while (prer_Kmerton_Run == true) {}                                  // Ждем окончания получения данных из Камертон
  regs_out[0] = 0x2B;                                                  // Код первого байта подключения к Камертону 43
...
void measure_vol_max(int istochnik, unsigned int adr_count, unsigned int adr_count200 , int adr_flagErr, unsigned int porogV, unsigned int porogVmax )
{
  ...
  switch (_adr_flagErr)
  {
    ...
    case 335:
      ...
      break;
    case 336:
      strcpy_P(buffer, (char*)pgm_read_word(&(string_table_err[129])));   // "Test Microphone ** Signal mag radio                          ON  - ";
      break;
  }
...
void measure_volume(int analog)
{
  ...
  voltage = volume_fact * (5.0 / 1024.0);
  if (analog == analog_FrontL )    voltage = volume_fact * (5.0 / 1024.0);
  if (analog == analog_FrontR )    voltage = volume_fact * (5.0 / 1024.0);
  if (analog == analog_LineL )     voltage = volume_fact * (5.0 / 1024.0);
  if (analog == analog_LineR )     voltage = volume_fact * (5.0 / 1024.0);
  if (analog == analog_mag_radio ) voltage = volume_fact * (5.0 / 1024.0);
  if (analog == analog_mag_phone ) voltage = volume_fact * (5.0 / 1024.0);
  if (analog == analog_ggs )       voltage = volume_fact * (5.0 / 1024.0 * 2.963);
  if (analog == analog_gg_radio1 ) voltage = volume_fact * (5.0 / 1024.0 * 2.963);
  if (analog == analog_gg_radio2 ) voltage = volume_fact * (5.0 / 1024.0 * 2.963);
...
void model_Audio1_2()
{
  ...
  byte i52 = regBank.get(40006);
    if (bitRead(i52, 0) == 0)                            // sensor подключения Mag
    { 
		save_reg_eeprom(adr_reg40014, 1);
		regBank.set(40014,1);                            // Версия платы Аудио1 или Аудио2
		Serial.print("Module Audio");
		Serial.println(regBank.get(40014));
	}
	else
	{
		save_reg_eeprom(adr_reg40014, 2);
		regBank.set(40014,2);                            // Версия платы Аудио1 или Аудио2
		Serial.print("Module Audio");
		Serial.println(regBank.get(40014));
	}
  regBank.set(21,1);                                     // XP2-2     sensor "Маг."
...
void version_ini_porog()
{
  int vers_ini  = i2c_eeprom_read_byte(deviceaddress, adr_vesion_Audio1_2);   // Считать из памяти версию загруженного порога
  regBank.set(40015,vers_ini);                            // Записать в регистр номер версии порогов
  delay(50);
  regBank.set(adr_control_command, 0);
}
...
void default_mem_porog()                                                    // Запись заводских установок уровней порога
{
  ...
  model_Audio1_memory();                                                  // Определить модель модуля Аудио
  if(regBank.get(40014)== 1)
	{
	  i2c_eeprom_write_byte(deviceaddress, adr_vesion_Audio1_2, 1);       // Записать в память номер версии порогов
	  regBank.set(40015,1);
	  Serial.println("Save porog Audio1");
		 for (int i = 0; i < _step_mem; i++)
		  {
			_u_porog = pgm_read_word_near(porog_default1 + i);
			// разбираем _u_porog на byte
			hi = highByte(_u_porog);
			low = lowByte(_u_porog);
			//	// тут мы эти hi,low можем сохранить EEPROM
    ...
    {
	  i2c_eeprom_write_byte(deviceaddress, adr_vesion_Audio1_2, 2);         // Записать в память номер версии порогов
	  regBank.set(40015,2);
	  Serial.println("Save porog Audio2");
		 for (int i = 0; i < _step_mem; i++)
		  {
			_u_porog = pgm_read_word_near(porog_default2 + i);
			// разбираем _u_porog на byte
			hi = highByte(_u_porog);
			low = lowByte(_u_porog);
			//	// тут мы эти hi,low можем сохранить EEPROM
...
void read_version_default()
{
  byte version_def = i2c_eeprom_read_byte(deviceaddress, adr_vesion_Audio1_2);   
  if(version_def == 1)         // Версия порогов для Аудио1
  {
     regBank.set(40015,1);
  }
  else if(version_def == 2)    // Версия порогов для Аудио2
  {
     regBank.set(40015,2);
  }
  else if(version_def == 3)    // Версия порогов загружена из файла
  {
     regBank.set(40015,3);
  }
  else                         // Версия порогов не определена
  {
     regBank.set(40015,0);
  }
  regBank.set(adr_control_command, 0);                                            // Завершить программу
...
void set_mem_porog()
{
  ...
  */
  ...
  for (int i = 0; i < _step_mem; i++)                           // Копирование блока памяти в регистры.
  {
	...
	int vers_ini  = i2c_eeprom_read_byte(deviceaddress, adr_vesion_Audio1_2);   // Считать из памяти версию загруженного порога
    regBank.set(40015,vers_ini);                            // Записать в регистр номер версии порогов
	Serial.print("Version porog - ");
    Serial.println(vers_ini);  
	regBank.set(adr_control_command, 0);                          // Завершить программу
	delay(200);
}
void read_mem_porog()
...
void setup_regModbus()
{
  ...
  */
  ...
  regBank.add(338);                         // Свободен
  regBank.add(339);                         // Свободен
  ...
  regBank.add(40014);  // Версия платы Аудио1 или Аудио2
  regBank.add(40015);  //
  regBank.add(40016);  //
  regBank.add(40017);  //
  regBank.add(40018);  //
...
void setup()
{
  ...
  for (int i = 120; i <= 141; i++)                             // Очистить флаги ошибок
  {
    regBank.set(i, 0);
  }
