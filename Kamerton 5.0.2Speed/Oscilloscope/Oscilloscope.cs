...
*************************************************************************
   ...
   Отображает осциллографа. После создания экземпляра не будет
    становятся видимыми, пока этот метод не называется.
   Lifetime:   instance
   Parameters: void
   Return:     void
...
*************************************************************************
   ...
   EXTERNAL_TRIG.
    Посылает новую точку данных на вход внешнего триггера,
 * который используется для запуска, когда осциллограф
 * Источник запуска установлен на внешние TRIG.
...
*************************************************************************
   ...
   in the dta stream.
  Быстро обновляет следы осциллографа,
 * для использования, когда есть разрыв в потоке данных.
   Lifetime:   instance
   Parameters: void
   Return:     void
...
*************************************************************************
   ...
   Освобождает ресурсы осциллографа обратно в систему и снимает осциллограф с дисплея. 
 * Также устанавливает расположенную флаг правда.
 * Любые будущие вызовы или доступы собственности будут игнорироваться и установить SCOPE_DISPOSED_ERROR в LastError.
   Lifetime:   instance
   Parameters: void
   Return:     void
...
**************************************************************************
   ...
   Oscilloscope class.
    Если установлено значение "истина", бросок появляется новое исключение, когда есть ошибка. 
 * При отладке завершено это может быть желательно установить сообщения об ошибках "ложного".
 * Последняя переменная
 * Ошибка будет работать независимо от этой настройки и могут быть проверены,
 * даже если нет экземпляр класса осциллограф. 
   Value:   bool
   Access:  Read/Write
   Default: true
...
**************************************************************************
   ...
   oscilloscope display.
     Отображает или скрывает панель управления в нижней части
    Осциллограф дисплей.
   Value:   bool
   Access:  Read/Write
...
**************************************************************************
   ...
   to the left edge of the oscilloscope form.
    Устанавливает горизонтальное положение осциллографа на экране,
    указано как число пикселей от левого края экрана
    к левому краю формы осциллографа.
...
**************************************************************************
   ...
   to the top edge of the oscilloscope form.
    Установка вертикального положения осциллографа на экране,
    указано как число пикселей от верхнего края экрана
    к верхнему краю формы осциллографа.
...
**************************************************************************
   ...
   Устанавливает ширину виде осциллографа в пикселях.
 * 
...
**************************************************************************
   ...
   Sets the Height of the oscilloscope form in pixels.
    Устанавливает высоту формы осциллографа в пикселях.
...
**************************************************************************
   ...
   and width of grid cells to this single property setting.
    Устанавливает размер ячейки сетки в пикселях; устанавливает и высоту
    и ширина ячеек сетки с этой установкой одной собственности.
...
**************************************************************************
   ...
   Установка заголовка в верхней части формы осциллографа.
...
**************************************************************************
   ...
   documentation).
     Устанавливает амплитуду Шкала следом 1 в сетки подразделений, в двоичном шагов сетки /
    клеток. Значение будет скорректирована с ближайшим действительным номером (см DLL
    документация).
...
**************************************************************************
   ...
   AmplitudeScale.
     Устанавливает вертикальное смещение следа 1, в единицах следа 1 в Amplitude Scale.
...
**************************************************************************
   ...
   RISING_EDGE_TRIG and DISABLED_TRIG are provided for convenience.
     Устанавливает режим триггера края обнаружения. Любое допустимое отрицательное INT задает он 
 * здоров фронту любая действительная положительная INT задает ему положительный фронту, и 0 отключаю. 
 * Константы FALLING_EDGE TRIG, RISING_EDGE TRIG и инвалидов TRIG предоставляются для удобства.
...
**************************************************************************
   ...
   Sets the Trigger Delay, in number of samples (see DLL Documentation).
     Устанавливает задержки триггера, в количестве образцов (см DLL документации).
...
**************************************************************************
   ...
   Returns the width of the current trace display area in pixels.
    Возвращает ширину текущего области отображения трассировки в пикселях. 
   Value:   int
   Access:  Read Only
   Default: N/A
...
**************************************************************************
   ...
   Returns the height of the current trace display area in pixels.
     Возвращает высоту текущего области отображения трассировки в пикселях.
   Value:   int
   Access:  Read Only
   Default: N/A
