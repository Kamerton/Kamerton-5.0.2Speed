...
volatile word samples[maxSamples];
volatile int channel=1, timeRes=200, a2d=1, timeStep=1, voltAmp=2, voltCoup=0, trigVal=0, trigMode=0;
...
{
  if (factor & 1) sbi(ADCSRA,ADPS0);
  else cbi(ADCSRA,ADPS0); 
  if (factor & 2) sbi(ADCSRA,ADPS1);
  else cbi(ADCSRA,ADPS1); 
  if (factor & 4) sbi(ADCSRA,ADPS0);
